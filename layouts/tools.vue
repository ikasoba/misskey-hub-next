			...
			>
				<nav class="border-r pr-2 py-5 flex flex-col w-72 overflow-y-scroll">
					<NuxtLink
						...
						:class="
							isSamePath(route.path, localePath('/tools/')) &&
				...
				<div class="lg:hidden bg-slate-100 dark:bg-slate-900">
					<button
						...
						class="bg-slate-300 dark:bg-slate-800 hover:bg-slate-400 dark:hover:bg-slate-700"
						:class="$style.toolsMenuToggle"
					>
						<span>{{ $t('_tools.menuToggle') }}</span>
					</button>
				</div>
			</aside>
			<main
				class="ml-8 lg:ml-72 lg:translate-x-0 transition-transform bg-slate-100 dark:bg-slate-900"
				:class="[isAsideNavOpen ? 'translate-x-72' : 'translate-x-0']"
			>
				<div :class="$style.slimPageRoot">
					<slot />
				</div>
				<GFooter
					class="mt-12 rounded-tl-3xl lg:rounded-tl-none bg-white dark:bg-slate-950"
				/>
			</main>
		</div>
	</div>
