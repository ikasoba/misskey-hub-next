...
const { data } = await useGAsyncData(`navigation_${locale.value}`, () =>
...
watch(locale, async (to) => {
	...
	const { data } = await useGAsyncData(`navigation_${to}`, () =>
		fetchContentNavigation(queryContent(`/${to}/docs/`)),
	);
	navigation.value = data.value;
	asideNavKey.value++;
});
...
<template>
	<div class="bg-white dark:bg-slate-950">
		...
		<div class="main-content">
			<div
				class="relative container mx-auto max-w-screen-xl p-6 lg:py-0 grid docs-root pb-12"
			>
				<div
				...
				>
					<div
						class="lg:sticky lg:top-16 h-[calc(100vh-7.25rem)] lg:h-[calc(100vh-4rem)] overflow-y-scroll border-r border-slate-200 dark:border-slate-700 py-6 pr-3"
					>
				...
				<div class="relative">
					...
					<div
						...
						:class="
							isAsideNavOpen ? 'opacity-70' : 'opacity-0 pointer-events-none'
						"
