<template>
	<div class="min-h-screen">
		<div class="sm:py-20 sm:max-w-xl mx-auto">
			<div
				class="p-6 bg-white dark:bg-gray-800 sm:rounded-lg min-h-screen sm:min-h-[230px]"
			>
				...
				<div v-else class="space-y-6">
					<div
						class="w-12 h-12 mx-auto bg-accent-600/20 text-accent-600 rounded-full p-3"
					>
						<component :is="branding?.icon ?? ShareIco" class="w-6 h-6" />
					</div>
					<h1 class="font-bold text-center text-lg md:text-xl">
						{{ branding?.heading ?? $t('_share.chooseServer') }}
					</h1>
					<ul
						class="rounded-lg border divide-y border-gray-300 dark:border-gray-600 divide-gray-300 dark:divide-gray-600"
					>
						<li
						...
						>
							...
							>
								<div
									...
									:class="
										instance.isUserDefined &&
										'transition-[border-color] hover:border-red-600 dark:hover:border-red-600'
									"
								>
									<template v-if="instance.isUserDefined">
										<button
											class="relative w-full h-full group/delete"
											@click.stop.prevent="deleteInstance(instance.url)"
										>
											<img
												v-if="instance.icon && instance.meta?.iconUrl"
												:src="getInstanceImage(instance)"
												class="w-full h-full"
											/>
											<div
												v-else-if="
													['forked', 'notDetermined'].includes(
														getPlaceholderImage(instance),
													)
												"
												class="w-full h-full bg-accent-600/20 p-2 text-accent-600"
											>
												<ForkedIco
													v-if="getPlaceholderImage(instance) === 'forked'"
													class="bi h-5 w-5 stroke-1 stroke-current"
												/>
												<QuestionIco v-else class="h-5 w-5" />
											</div>
											<img
												v-else
												:src="getPlaceholderImage(instance)"
												class="w-full h-full"
											/>
											<div
												class="pointer-events-none absolute top-0 left-0 w-full h-full bg-white dark:bg-gray-800 transition-opacity opacity-0 group-hover/delete:opacity-100 p-2 text-red-600"
											>
									...
									<img
										v-else-if="instance.icon && instance.meta?.iconUrl"
										:src="instance.meta?.iconUrl"
										class="w-full h-full"
									/>
								</div>
						...
						<li class="group">
							<details
								class="group-first:rounded-t-lg group-last:rounded-b-lg group/details"
							>
								<summary
									class="p-4 w-full flex items-center hover:bg-gray-100 dark:hover:bg-gray-700 group-first:rounded-t-lg group-last:rounded-b-lg group-last:group-open/details:rounded-b-none group-open/details:border-b cursor-pointer border-gray-300 dark:border-gray-600"
								>
									<div class="h-9 w-9 mr-2">
										<div
											class="w-full h-full rounded bg-accent-600/20 p-2 text-accent-600"
										>
									...
									<ArrowRightIco
										class="block ml-auto flex-shrink-0 h-4 w-4 transition-transform group-open/details:rotate-90"
									/>
								</summary>
								<div class="p-4">
									<form @submit.prevent="getAndSetInstanceInfo">
										<label
											class="form-label inline-block"
											for="userDefinedInstanceInput"
											>{{ $t('_share.domain') }}</label
										>
										<div class="input-group">
											<input
												id="userDefinedInstanceInput"
												class="form-control"
												autocomplete="off"
												placeholder="misskey.example.com"
												v-model="userDefinedInstanceInput"
												:disabled="iFetching"
											/>
											<button
												type="submit"
												...
												:disabled="iFetching"
											>
												<PlusIco class="h-4 w-4 stroke-1 stroke-current" />
											</button>
										</div>
										<div class="form-text">
											{{ $t('_share.compatibleWith') }}
										</div>
									</form>
								</div>
							</details>
					...
					<div class="text-sm text-center">
						...
						<GNuxtLink
							to="https://misskey-hub.net/"
							target="_blank"
							class="hover:underline underline-offset-1"
							>Misskey Hub</GNuxtLink
						>
					</div>
				</div>
			</div>
...
async function getAndSetInstanceInfo() {
	if (
		...
		!userDefinedInstanceInput.value.includes('.')
	)
		return;
	iFetching.value = true;

	nextTick(async () => {
		const realHost = parseURL(
			userDefinedInstanceInput.value.startsWith('https://')
				? userDefinedInstanceInput.value
				: 'https://' + userDefinedInstanceInput.value,
		);
		if (!realHost.host) {
...
function getPlaceholderImage(instance: ExtendedInstanceItem | InstanceItem) {
	if (instance.meta?.repositoryUrl) {
		if (
			forkedSoftwares.some(
				(v) => instance.meta?.repositoryUrl.toLowerCase().includes(v),
			)
		) {
			return 'forked';
		}
...
onMounted(async () => {
	if (process.client) {
		const fetchedInfo = await window.fetch(
		...
		featuredInstances.value = fetchedInfoJson.instancesInfos
			.sort((a, b) => {
				return resolveObjPath(a, 'stats.originalUsersCount') >
					resolveObjPath(b, 'stats.originalUsersCount')
					? -1
					: 1;
			})
			.slice(0, 5);
