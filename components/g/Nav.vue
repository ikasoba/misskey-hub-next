<template>
	...
	>
		<nav
			class="container mx-auto max-w-screen-xl grid items-center grid-cols-2 lg:grid-cols-6 p-4 h-full transition-[height]"
		>
			<div class="">
				<GNuxtLink
					:to="localePath('/')"
				...
				>
					...
					<div class="font-title font-bold text-lg">
						{{ $t('_seo.siteName') }}
					</div>
				</GNuxtLink>
			</div>
			<ul
				...
				:class="[
					scrollPos <= -40 ? 'rounded-bl-lg' : 'rounded-l-lg',
			...
			>
				<li v-for="item in NavData.center">
					<GNuxtLink
						...
						:class="[
							'block rounded-full px-4 py-2 lg:px-4 lg:py-1.5 hover:bg-slate-300 dark:hover:bg-slate-800',
							{
								'bg-slate-300 dark:bg-slate-800 font-bold':
				...
				<li class="lg:hidden px-4 py-2 flex justify-end items-center space-x-4">
					<button
						class="hover:opacity-80 relative before:absolute before:-z-10 before:-top-2 before:-left-2 before:w-9 before:h-9 before:rounded-full hover:before:bg-slate-300 dark:hover:before:bg-slate-600 h-5 w-5"
					...
					<div class="input-group">
						<span class="input-group-text !rounded-l-full"
							><I18nIcon class="h-5 w-5" /><span class="sr-only">{{
								$t('_nav.switchLang')
							}}</span></span
						>
						<select
							class="form-select !rounded-r-full"
							v-model="spLocaleOption"
							@change="changeLocale()"
						>
				...
				<ul class="hidden lg:col-span-4 lg:space-x-4 lg:flex justify-center">
					<li>
						<button
							:class="[
								'hover:opacity-80',
								{
									'text-white 3xl:text-slate-800 3xl:dark:text-slate-200':
										landing && scrollPos >= -40,
								},
							]"
							@click="rotateColorMode()"
							aria-label="Change Color Mode"
					...
					<li class="relative group">
						<button class="hover:opacity-80">
							...
							/><span class="sr-only">{{ $t('_nav.switchLang') }}</span>
						</button>
						<div
							class="absolute top-6 right-0 hidden group-hover:block z-[9955]"
						>
							<ul
								class="px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg shadow-lg space-y-1"
							>
								<li v-for="locale in locales">
									<GNuxtLink
										...
										:lang="locale.code"
										:class="[
											'_i18n whitespace-nowrap hover:text-accent-600 py-1',
											{
