<template>
	<div
		:class="['grid grid-cols-1 md:grid-cols-2 gap-4', wide && 'lg:grid-cols-3']"
	>
		<ContentNavigation v-slot="{ navigation }" :query="query">
			<GNuxtLink
				...
				v-for="item in findDeepObject(
					navigation[0] as Record<string, any>,
					(v) =>
						realBasePath.replace(/\/$/, '') === v?._path.replace(/\/$/, ''),
				)?.children ?? []"
				:key="item._path"
				:to="item._path"
			>
				...
				<p
					class="text-sm text-slate-500 dark:text-slate-400 !mb-0 truncate-box"
				>
					{{ item.description ?? '' }}
				</p>
			</GNuxtLink>
