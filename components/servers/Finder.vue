<template>
	<div class="container mx-auto max-w-screen-xl px-6 grid server-list gap-8">
		...
		>
			...
			<div
				class="lg:sticky lg:top-24 lg:overflow-y-auto lg:max-h-[calc(100vh-6rem)] p-6 lg:px-0 lg:py-2 space-y-4"
			>
				<div class="flex items-center">
					<h3 class="text-xl font-bold">{{ $t('_servers._search.title') }}</h3>
					<button
						@click="sortOpen = false"
						class="ml-auto w-8 h-8 p-0.5 rounded-full bg-slate-100 dark:bg-slate-900 lg:hidden"
					>
						<XIco class="w-7 h-7" />
					</button>
				</div>
				<form
					@submit.prevent="
						() => {
							f_query = f_query_partial;
						}
					"
				>
					<label class="form-label" for="query">{{
						$t('_servers._search.query')
					}}</label>
				...
				<div>
					<label class="form-label" for="orderBy">{{
						$t('_servers._search.orderBy')
					}}</label>
					<div class="input-group">
						<select id="orderBy" v-model="f_orderBy" class="form-select">
							<option value="recomendded">
								{{ $t('_servers._search.recomendded') }}
							</option>
							<option value="notesPer15Days">
								{{ $t('_servers._search.notesPer15Days') }}
							</option>
							<option value="notesCount">
								{{ $t('_servers._search.notesCount') }}
							</option>
							<option value="usersCount">
								{{ $t('_servers._search.usersCount') }}
							</option>
						</select>
						<button class="btn btn-outline-primary" @click="switchOrder()">
							<SortDownIco
								v-if="f_order === 'desc'"
								class="stroke-[0.5] stroke-current"
							/>
				...
				<div>
					...
					<div class="form-check">
						<input
							...
							type="radio"
							name="registerAcceptance"
							v-model="f_registerAcceptance"
							:value="null"
							id="registerAcceptance0"
						/>
				...
				<div class="btn-group w-full" role="group">
					<input
						...
						v-model="v_view"
					/>
					<label class="btn btn-outline-primary truncate" for="btnradio1"
						><GridIco class="mr-1" />{{ $t('_servers._view.grid') }}</label
					>
					...
					<input
						...
						v-model="v_view"
					/>
					<label class="btn btn-outline-primary truncate" for="btnradio2"
						><ListIco class="mr-1" />{{ $t('_servers._view.list') }}</label
					>
				</div>
			</div>
		</aside>
		<div>
			...
			>
				<ServersItem
					...
					v-for="item in filteredInstances.slice(0, f_limit)"
					:instance="item"
					:view="v_view"
				/>
				<div
				...
				<button
					v-if="f_limit < filteredInstances.length"
					@click="f_limit += 24"
...
if (process.client) {
	savedSettings = JSON.parse(
		window.localStorage.getItem('miHub_server_finder') ?? 'null',
	) as MiHubSFStorage | null;
}
...
const f_orderBy = ref<MiHubSFStorage['f_orderBy']>(
	savedSettings?.f_orderBy ?? 'recomendded',
);
const f_order = ref<MiHubSFStorage['f_order']>(
	savedSettings?.f_order ?? 'desc',
);
const f_registerAcceptance = ref<MiHubSFStorage['f_registerAcceptance']>(
	savedSettings?.f_registerAcceptance || null,
);
...
// ▼フォームデータ保存処理▼
watch(
	[f_langs, f_orderBy, f_order, f_registerAcceptance, v_view],
	(to, from) => {
		f_limit.value = 24;
...
const { data } = await useFetch<InstanceInfo>(
...
const filteredInstances = computed<InstanceItem[]>(() => {
	...
	if (f_query.value) {
		instances = instances.filter(
			(instance) =>
				instance.name.includes(f_query.value) ||
				instance.description?.includes(f_query.value),
		);
	}
	if (f_langs.value) {
		instances = instances.filter((instance) =>
			instance.langs.includes(f_langs.value),
		);
	}
	...
	if (f_orderBy.value) {
		instances.sort((a, b) => {
			...
			if (f_order.value === 'desc') {
				return resolveObjPath(a, orderKey) > resolveObjPath(b, orderKey)
					? -1
					: 1;
			} else {
				return resolveObjPath(a, orderKey) < resolveObjPath(b, orderKey)
