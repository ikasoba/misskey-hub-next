<template>
	<div class="container mx-auto max-w-screen-lg px-6 space-y-6 lg:space-y-8">
			...
			<div class="grid gap-4 md:grid-cols-2">
				<div>
					<ChartsCircGraph
						...
						:data="langStats"
						:focusedIndex="langFocus"
					/>
				</div>
				<div>
					<ul class="space-y-1">
						<li
							v-for="(value, key, index) in trunc(langStats)"
							class="grid py-1 px-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
							:class="$style.kvRoot"
							@mouseenter.passive="langFocus = index"
							@mouseleave.passive="langFocus = undefined"
						>
							<div v-if="key === '__others'">{{ $t('other') }}</div>
							<div v-else>
								{{ key }}
								{{
									lang.find((v) => v.lang === key)?.label
										? `(${lang.find((v) => v.lang === key)?.label})`
										: ''
								}}
							</div>
							<div class="font-bold font-mono text-accent-600">
								{{ $n(value) }}
							</div>
						</li>
					</ul>
		...
		<div id="registerAcceptance">
			<h2 class="text-2xl lg:text-3xl font-title font-bold mb-4">
				{{ $t(`_servers._statistics.registerAcceptance`) }}
			</h2>
			<div class="grid gap-4 md:grid-cols-2">
				<div>
					<ChartsCircGraph
						class="max-w-xs mx-auto"
						:data="regAcceptanceStats"
						:focusedIndex="regAcceptanceFocus"
						:truncMinor="false"
					/>
				</div>
				<div>
					<ul class="space-y-1">
						<li
							v-for="(value, key, index) in regAcceptanceStats"
							class="grid py-1 px-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
							:class="$style.kvRoot"
							@mouseenter.passive="regAcceptanceFocus = index"
							@mouseleave.passive="regAcceptanceFocus = undefined"
						>
		...
		<div id="notes">
			...
			<div class="grid gap-4 md:grid-cols-2">
				<div>
					<ChartsCircGraph
						class="max-w-xs mx-auto"
						:data="notesCountStats"
						:focusedIndex="notesCountFocus"
					/>
				</div>
				<div>
					<ul class="space-y-1">
						<li
							v-for="(value, key, index) in trunc(notesCountStats)"
							class="grid py-1 px-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
							:class="$style.kvRoot"
							@mouseenter.passive="notesCountFocus = index"
							@mouseleave.passive="notesCountFocus = undefined"
						>
							<div v-if="key === '__others'">{{ $t('other') }}</div>
							<div v-else>{{ key }}</div>
							<div class="font-bold font-mono text-accent-600">
		...
		<div id="version">
			<h2 class="text-2xl lg:text-3xl font-title font-bold mb-4">
				{{ $t(`_servers._statistics.version`) }}
			</h2>
			<div class="grid gap-4 md:grid-cols-2">
				<div>
					<ChartsCircGraph
						class="max-w-xs mx-auto"
						:data="avgVersionStats"
						:truncMinor="0.005"
						:focusedIndex="avgVersionFocus"
					/>
				</div>
				<div>
					<ul class="space-y-1">
						<li
							v-for="(value, key, index) in trunc(avgVersionStats, 0.005)"
							class="grid py-1 px-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
							:class="$style.kvRoot"
							@mouseenter.passive="avgVersionFocus = index"
							@mouseleave.passive="avgVersionFocus = undefined"
						>
							<div v-if="key === '__others'">{{ $t('other') }}</div>
							<div v-else>{{ key }}</div>
							<div class="font-bold font-mono text-accent-600">
