<template>
	<svg viewBox="-3 -3 70 70">
		<circle
			...
			cy="32"
			r="16"
			:class="[$style.pie, focusedIndex === index && $style.focused]"
...
const steppedColors = computed(() => {
	const r = sortedData.value.map(
		(_, i, a) =>
			props.startColor[0] -
			((props.startColor[0] - props.endColor[0]) / a.length) * i,
	);
	const g = sortedData.value.map(
		(_, i, a) =>
			props.startColor[1] -
			((props.startColor[1] - props.endColor[1]) / a.length) * i,
	);
	const b = sortedData.value.map(
		(_, i, a) =>
			props.startColor[2] -
			((props.startColor[2] - props.endColor[2]) / a.length) * i,
	);
