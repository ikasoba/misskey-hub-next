<template>
	<ul
		class="toc-links text-sm"
		:class="[`depth-${depth}`, depth === 1 ? 'mb-4' : 'mb-2']"
	>
		<li
		...
		>
			...
			>
				<div class="flex">
					<button
						v-if="
							link.children &&
							link.children.filter((v) => !isSamePath(v._path, link._path))
								.length > 0
						"
						...
						@click.prevent.stop="
							() => {
								...
								manualOpen[link._path] = !(
									manualOpen[link._path] ?? path.includes(link._path)
								);
							}
						"
					>
			...
			<AsideNav
				v-if="
					link.children &&
					link.children.filter((v) => !isSamePath(v._path, link._path)).length >
						0
				"
				v-show="
					(path.includes(link._path) && manualOpen[link._path] !== false) ||
					manualOpen[link._path]
				"
				:links="[link]"
				:depth="depth + 1"
...
const manualOpen = useState<Record<string, boolean>>(
	'miHub-docs-aside-manual-collapse',
	() => ({}),
);
