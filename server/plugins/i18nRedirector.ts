...
export default defineNitroPlugin((nitroApp) => {
	nitroApp.hooks.hook('render:html', (html, { event }) => {
		if (
			event.path.includes('_nuxt') ||
			event.path.match(/\d{3}\.html/) ||
			event.path.match(/^\/ns/)
		) {
			return;
		}
		...
		if (
			...
			const links = locales.map((l) => {
				const url = withTrailingSlash(
					`/${l.code}${event.path.replace(/\.html$/g, '/')}`,
				);
			...
			html.head.forEach((v) => {
				...
				remainingList.push(
					...(v.match(/<link\s+rel="(og|alternate|canonical)[^>]+>/gm) ?? []),
				);
			...
			//@ts-ignore
			html.head.push(
				'<script type="text/javascript">const s = ' +
					JSON.stringify(locales.map((l) => l.code)) +
