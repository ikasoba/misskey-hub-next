...
const getTitle = () =>
	route.meta.title
		? `${route.meta.title} | ${t('_seo.siteName')}`
		: t('_seo.siteNameLong');
		...
		'@graph': [
			{
				...
				name: 'Misskey',
				url: `${baseUrl}/`,
				sameAs: ['https://ja.wikipedia.org/wiki/Misskey'],
				logo: {
					'@type': 'ImageObject',
...
const currentLocaleIso = computed(
	() =>
		(locales.value as LocaleObject[]).find((e) => e?.code === locale.value)
			?.iso,
);
...
const i18nLinks = computed(
	() =>
		head.value.link?.map((e) => {
			if (e.rel === 'alternate') {
				let href = e.href;
				if (
					typeof e.hreflang === 'string' &&
					(e.hreflang.includes('ja') || e.hreflang === 'x-default')
				) {
...
const cnHead =
	locale.value === 'cn'
		? [
				...
				{
					rel: 'stylesheet',
					href: 'https://fonts.googleapis.cn/css2?family=Capriola&family=Nunito:ital,wght@0,400;0,700;1,400;1,700&display=swap',
				},
			]
		: [
				...
				{
					rel: 'stylesheet',
					href: 'https://fonts.googleapis.com/css2?family=Capriola&family=Nunito:ital,wght@0,400;0,700;1,400;1,700&display=swap',
				},
			];
	...
	(): Record<string, any> => ({
		...
		meta: [
			...
			},
			{
				...
				// TODO
				content: () =>
					route.meta.thumbnail
						? route.meta.thumbnail
						: `${baseUrl}/img/og/misskey-hub-screenshot-l.png`,
			},
			...(head.value.meta?.map((e) => ({
				property: e.property,
				content: e.content,
			})) || []),
		],
		link: [...(i18nLinks.value || []), ...cnHead],
		script: [
...
const hideFrom = computed(() =>
	route.meta.scrollButton ? route.meta.scrollButton?.hideFrom ?? -45 : -45,
);
const sbPosition = computed(() =>
	route.meta.scrollButton
		? {
				x: route.meta.scrollButton?.customPosition?.x ?? '2.5rem',
				y: route.meta.scrollButton?.customPosition?.y ?? '2.5rem',
			} ?? { x: '2.5rem', y: '2.5rem' }
		: { x: '2.5rem', y: '2.5rem' },
);
...
<template>
	<div class="text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-gray-900">
		...
		<button
			...
			@click="scrollToTop"
		>
			<TopIco class="h-5 w-5 stroke-1 stroke-current" />
		</button>
	</div>
</template>
