<template>
	<div>
		<div class="mx-auto container max-w-screen-md relative px-6 py-5">
			<GNuxtLink
				:to="localePath('/blog/', originalLocale)"
				class="absolute top-2.5 left-6 p-2.5 opacity-40 hover:opacity-100 hover:-translate-x-2.5 group transition-[opacity,transform] flex items-center"
			>
			...
			<h1 class="text-center font-bold text-2xl lg:text-3xl mb-4">
				{{ data?.title }}
			</h1>
			...
			<div class="text-center mt-6 lg:mt-12">
				...
				<div class="mb-6 flex items-center justify-center w-full space-x-4">
					<GNuxtLink
						:to="miShareUrl"
						target="_blank"
						class="h-12 p-3 flex items-center rounded-full bg-accent-600 hover:opacity-80 text-white"
						><MiIco class="w-7 h-7" /><span class="sr-only">Misskey</span>
						<div class="ml-1.5 font-bold">{{ $t('note') }}</div></GNuxtLink
					>
					<GNuxtLink
						:to="mtdShareUrl"
						target="_blank"
						class="w-12 h-12 p-3 rounded-full bg-[#563ACC] hover:opacity-80 text-white"
						><MastoIco class="w-6 h-6" /><span class="sr-only"
							>Mastodon</span
						></GNuxtLink
					>
					<GNuxtLink
						:to="twShareUrl"
						target="_blank"
						class="w-12 h-12 p-3 rounded-full bg-black dark:bg-gray-700 hover:opacity-80 text-white"
						><TwitterXIco class="w-6 h-6" /><span class="sr-only"
							>X (Twitter)</span
						></GNuxtLink
					>
				</div>
...
const { data } = await useGAsyncData(`blog-${route.params.slug}`, () =>
	queryContent<MiBlogParsedContent>(`/blog/${route.params.slug}`).findOne(),
);
...
const thumbnail = !data.value.thumbnail
	? undefined
	: parseURL(data.value.thumbnail).host == null
		? joinURL(runtimeConfig.public.baseUrl, data.value.thumbnail)
...
const pd = data.value.date
	? new Date(data.value.date).toISOString()
	: undefined;

const miShareUrl = computed(() =>
	withQuery('https://misskey-hub.net/share', {
		text: route.meta.title + ' - Misskey Hub',
		url: `${runtimeConfig.public.baseUrl}/ja/blog/${route.params.slug}/`,
	}),
);

const mtdShareUrl = computed(() =>
	withQuery('https://donshare.net/share.html', {
		text: route.meta.title + ' - Misskey Hub',
		url: `${runtimeConfig.public.baseUrl}/ja/blog/${route.params.slug}/`,
	}),
);

const twShareUrl = computed(() =>
	withQuery('https://twitter.com/share', {
		text: route.meta.title + ' - Misskey Hub',
		url: `${runtimeConfig.public.baseUrl}/ja/blog/${route.params.slug}/`,
	}),
);
