<template>
	<div>
				...
				<div class="hidden lg:block relative px-6 py-8">
					...
					<div class="relative lg:w-64">
						<img
							...
							src="/img/emojis/open_mailbox_with_raised_flag_3d.png"
						/>
					</div>
				</div>
			</template>
		...
		<div class="pb-12 lg:mt-12 pt-6 bg-white dark:bg-slate-950">
			<div
				class="container mx-auto max-w-screen-lg px-6 space-y-4 lg:space-y-2"
			>
				<GNuxtLink
...
const { data } = await useGAsyncData('blog', () =>
	queryContent('blog')
		.only(['_path', 'navTitle', 'title', 'date'])
		.sort({ date: -1 })
		.find(),
);
