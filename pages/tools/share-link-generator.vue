		...
		<div class="rounded-lg grid md:grid-cols-2 gap-8">
			<div class="space-y-4">
				<div class="p-6 space-y-4 rounded-lg bg-white dark:bg-slate-950">
					...
					<div>
						<div class="flex">
							<label for="shareLinkGeneratorBody">{{
								$t('_shareLinkGenerator.body')
							}}</label>
							<div
								class="ml-auto"
								:class="[
									(mfmText?.length ?? 0) >= 5000 && 'font-bold text-red-600',
								]"
							>
						...
						></textarea>
						<div class="form-text">
							{{ $t('_shareLinkGenerator.bodyWarning') }}
						</div>
					</div>
					<div>
						<label for="shareLinkGeneratorUrl">{{
							$t('_shareLinkGenerator.url')
						}}</label>
						<input
							class="form-control"
							id="shareLinkGeneratorUrl"
							v-model="mfmUrl"
						/>
						<div class="form-text">
							{{ $t('_shareLinkGenerator.urlCaption') }}
						</div>
					</div>
					<div>
						...
						<div
							:class="$style.mfmRoot"
							class="mb-2 bg-white dark:bg-[#212529] border-gray-200 dark:border-gray-600"
						>
				...
				<div class="p-6 space-y-4 rounded-lg bg-white dark:bg-slate-950">
					...
					<div>
						<label for="shareLinkGeneratorVisibility">{{
							$t('_shareLinkGenerator.visibility')
						}}</label>
						<select
							class="form-select"
							id="shareLinkGeneratorVisibility"
							v-model="visibility"
						>
							<option
								v-for="visibility in noteVisibilities"
								:key="visibility"
								:value="visibility"
							>
								{{ $t(`_noteVisibility.${visibility}`) }}
							</option>
						</select>
					</div>
					<div v-if="visibility === 'specified'">
						<label for="shareLinkGeneratorRecipents">{{
							$t('_shareLinkGenerator.recipents')
						}}</label>
						<textarea
							class="form-control"
					...
					<div class="form-check">
						<input
							...
							type="checkbox"
							v-model="localOnly"
							id="shareLinkGeneratorLocalOnly"
						/>
			...
			<div class="space-y-4">
				<div class="p-6 space-y-4 rounded-lg bg-white dark:bg-slate-950">
					...
					<div
						v-if="(mfmText?.length ?? 0) === 0"
						class="p-8 text-center font-bold text-lg"
					>
					...
					<template v-else>
						...
						<div class="form-text">
							<GNuxtLink
								...
								>{{ $t('_shareLinkGenerator.testLink') }}<ExtIco class="ml-1"
							/></GNuxtLink>
						</div>
					</template>
				</div>
