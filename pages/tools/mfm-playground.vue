	...
	<div class="container mx-auto max-w-screen-xl px-6 py-6">
		...
		<div class="rounded-lg grid md:grid-cols-2 gap-4">
			...
			<div class="space-y-4">
				...
				<div>
					<label for="mfmPlaygroundDomain">{{
						$t('_mfmPlayground.domain')
					}}</label>
					<div class="input-group">
						<input
							type="text"
							class="form-control"
							id="mfmPlaygroundDomain"
							v-model="mfmHost"
						/>
						<GNuxtLink :to="shareURL" target="_blank" class="btn btn-primary"
							>{{ $t('_mfmPlayground.noteIt') }}<SendIco class="ml-1"
						/></GNuxtLink>
					</div>
				</div>
				<div>
					<div class="flex">
						...
						<div class="w-1/2">
							<button
								...
								class="btn w-full btn-outline-primary"
							>
								{{ $t('_mfmPlayground.clearEmojiCache') }}
							</button>
						</div>
					</div>
				</div>
...
const shareURL = computed(() => {
	const domain =
		'https://' + (parseURL(mfmHost.value).host ?? mfmHost.value) + '/share';
