<template>
	<div class="container mx-auto max-w-screen-xl px-6 py-6">
		...
		<div class="grid grid-cols-2 gap-2 md:gap-8 mx-auto max-w-lg mb-4">
			<button
				:class="[
					'rounded-full py-2 hover:bg-accent-600/60',
					{ 'bg-accent-600/40': tab === 'aidToDate' },
				]"
				@click="tab = 'aidToDate'"
			>
				{{ $t('_aidConverter.aidToDate') }}
			</button>
			<button
				:class="[
					'rounded-full py-2 hover:bg-accent-600/60',
					{ 'bg-accent-600/40': tab === 'dateToAid' },
				]"
				@click="tab = 'dateToAid'"
			>
				{{ $t('_aidConverter.dateToAid') }}
			</button>
		</div>
		<div v-if="tab === 'aidToDate'" class="mx-auto max-w-lg">
			...
			<div class="my-2">
				<button class="btn btn-primary" @click="doAidToDate()">
					{{ $t('_aidConverter.aidToDate') }}
				</button>
			</div>
			<div
				class="mb-2 p-4 rounded-lg border bg-white dark:bg-[#212529] border-gray-200 dark:border-gray-600"
			>
				{{ aidToDateResult }}
			</div>
		</div>
		<div v-if="tab === 'dateToAid'" class="mx-auto max-w-lg">
			<label class="mb-1" for="dateToAidDate">{{
				$t('_aidConverter.date')
			}}</label>
			<input
				class="form-control mb-2"
				id="dateToAidDate"
				type="datetime-local"
				v-model="dateToAidDate"
			/>
			<label class="mb-1" for="DateToAidMode">{{
				$t('_aidConverter.mode')
			}}</label>
			...
			<div class="my-2">
				<button class="btn btn-primary" @click="doDateToAid()">
					{{ $t('_aidConverter.dateToAid') }}
				</button>
			</div>
			<div
