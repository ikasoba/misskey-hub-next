		...
		<GHero>
			...
			<template #description>
				...
				<I18nT keypath="_servers.addYourServer" tag="span">
					<GNuxtLink
						class="font-bold hover:underline underline-offset-4"
						to="https://github.com/joinmisskey/api"
						>{{ $t('_servers.addYourServerLink') }}</GNuxtLink
					>
				</I18nT>
			</template>
			<template #icon>
				<div class="relative px-6 py-8">
					...
					<div
						class="relative bg-white dark:bg-slate-800 shadow-lg rounded-lg w-full lg:w-80 p-6 space-y-4 break-words"
					>
						<dl>
							<dt>{{ $t('_servers._statistics.notes') }}</dt>
							<dd class="font-bold text-accent-600 text-2xl">
								{{
									instancesStats?.notesCount
										? $n(instancesStats.notesCount)
										: $t('loading')
								}}
							</dd>
						</dl>
						<dl>
							...
							<dd class="font-bold text-accent-600 text-2xl">
								{{
									instancesStats?.usersCount
										? $n(instancesStats.usersCount)
										: $t('loading')
								}}
							</dd>
						</dl>
						<dl>
							...
							<dd class="font-bold text-accent-600 text-2xl">
								{{
									instancesStats?.instancesCount
										? $n(instancesStats.instancesCount)
										: $t('loading')
								}}
							</dd>
						</dl>
						<div class="!mt-2">
							<GNuxtLink
								...
								:to="localePath('/servers/stats/')"
								>{{ $t('_servers._statistics.viewFullStats')
								}}<ArrowRightIco class="ml-1"
							/></GNuxtLink>
						</div>
					</div>
				</div>
