				...
				<div class="hidden lg:block relative px-6 py-8">
					...
					<div class="relative lg:w-64 h-64 flex items-center">
						<img
							...
							src="/img/emojis/technologist_3d.png"
						/>
					</div>
				</div>
			</template>
			...
			<div class="container mx-auto max-w-screen-xl px-6 pt-4 space-y-8">
				<div class="grid gap-x-12 gap-y-4 w-full" :class="$style.teamRoot">
					<div>
						<div class="lg:sticky lg:top-32">
							<h3
								class="font-title font-bold text-2xl lg:text-2xl mb-2 lg:mb-4"
							>
					...
					<div>
						<div class="grid md:grid-cols-2 gap-4">
							<AboutUsTeamMember
								v-for="member in coreTeamMember"
								:member="member"
							/>
						</div>
					</div>
				...
				<div class="grid gap-x-12 gap-y-4 w-full" :class="$style.teamRoot">
					...
					<div>
						<div class="grid md:grid-cols-2 gap-4">
							<AboutUsTeamMember
								v-for="member in coreTeamEmeriti"
								:member="member"
							/>
						</div>
					</div>
				...
				<div class="grid gap-x-12 gap-y-4 w-full" :class="$style.teamRoot">
					...
					<div>
						<div class="grid md:grid-cols-2 gap-4">
							...
							/>
							<GNuxtLink
								to="https://github.com/misskey-dev/misskey/graphs/contributors"
								target="_blank"
								class="block p-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-lg"
							>
...
const { data } = await useFetch<{ contributors: MiHubMember[] }>(
	'https://ungh.cc/repos/misskey-dev/misskey/contributors',
	{
		onRequestError: () => {
...
const contributors = computed(
	() =>
		data.value?.contributors.filter(
			(v) =>
				!v.username?.includes('[bot]') &&
				!coreTeamMember.map((e) => e.id).includes(v.id) &&
				!coreTeamEmeriti.map((e) => e.id).includes(v.id),
		),
);
